...
class BankReport {
    ...
    public static int getNumberOfCustomers(Bank bank) {
        ...
        Integer numberOfCustomers = bank.getClients()
                .stream()
                .map(x -> x.getEmployees().size())
    ...
    public static int getNumberOfAccounts(Bank bank) {
        ...
        List<Business> businesses = Arrays.asList(bank.getClients().toArray(new Business[0]));
        List<Set<Employee>> employeesSet =
                businesses.stream().map(x -> x.getEmployees()).collect(Collectors.toList());
        List<Employee> employees =
                employeesSet.stream().flatMap(Set::stream).collect(Collectors.toList());
        Integer numberOfAccounts = employees.stream().map(x -> x.getAccounts().size()).reduce(0, Integer::sum);
    ...
    public static SortedSet<Employee> getCustomersSorted(Bank bank) {
        ...
        TreeSet<Employee> customersSorted = new TreeSet<>(new Comparator<Employee>() {
            @Override
        ...
        customersSorted.addAll(employees.stream().sorted(customersSorted.comparator()).collect(Collectors.toSet()));
    ...
    public static double getTotalSumInAccounts(Bank bank) {
        ...
        List<Employee> employees =
                employeesSet.stream().flatMap(Set::stream).collect(Collectors.toList());
        List<Set<Account>> accountsSet =
                employees.stream().map(x -> x.getAccounts()).collect(Collectors.toList());
        List<Account> accounts =
                accountsSet.stream().flatMap(Set::stream).collect(Collectors.toList());

        Optional<Double> totalSumInAccounts =
                accounts.stream().map(x -> x.getBalance()).reduce(Double::sum);

        return totalSumInAccounts.get();
    }
    ...
    public static SortedSet<Account> getAccountsSortedBySum(Bank bank) {
        ...
        TreeSet<Account> accountsSorted = new TreeSet<>(new Comparator<Account>() {
            @Override
        ...
        accountsSorted.addAll(accounts.stream().sorted(accountsSorted.comparator()).collect(Collectors.toSet()));
    ...
    public static Map<Employee, Collection<Account>> getCustomerAccounts(Bank bank) {
        ...
        Map<Employee, Collection<Account>> map = employees.stream().collect(Collectors.toMap(employee -> employee,
                        employee -> employee.getAccounts()));
...
class BusinessReport {
    ...
    public static Map<Disability, List<Employee>> getEmployeesOnSameDisability(Business business) {
        ...
        List<Employee> employees = business.getEmployees().stream().collect(Collectors.toList());

        Map<Disability, List<Employee>> map = employees.stream().collect(Collectors.groupingBy(Employee::getDisability));
    ...
    public static long getNumberOfDifferentProjectsWorkedByCurrentFemaleEmployees(Business business) {
        ...
        List<Employee> currentFemaleEmployees = employees.stream().filter(x -> x.getGender().equals(Gender.FEMALE)).collect(Collectors.toList());

        List<List<Project>> projectsList = currentFemaleEmployees.stream().map(x -> x.getProjects()).collect(Collectors.toList());

        List<Project> projects =
                projectsList.stream().flatMap(List::stream).distinct().collect(Collectors.toList());
    ...
    public static Map<Religion, Map<Gender, List<Employee>>> getEmployeesOnSameReligionAndGender(Business business) {
        ...
        Map<Religion, Map<Gender, List<Employee>>> map = employees.stream().collect(Collectors.groupingBy(Employee::getReligion, Collectors.groupingBy(Employee::getGender)));
